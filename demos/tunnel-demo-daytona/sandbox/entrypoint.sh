#!/bin/sh
# Sandbox entrypoint for the 3-tunnel demo. Starts a local peer-only daemon
# (same pattern as ../../daytona-poc/entrypoint.sh), installs the caller's
# public key, starts sshd, starts a static web server for the placeholder
# page, then advertises all three (sshd, web server, this daemon's own
# dashboard) over Datum peer tunnels so the laptop can `peer connect` to
# each of them. Everything keeps running in the background after this
# script exits -- Daytona sandboxes are long-running, not batch jobs.
set -e
...
echo "=== [4/5] starting the placeholder web page ==="
...
advertise 127.0.0.1:22 ssh-demo ssh
advertise 127.0.0.1:8899 web-demo web
advertise 127.0.0.1:47780 dashboard-demo dashboard
